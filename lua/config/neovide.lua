...
local function get_max_refresh_rate()
...
local function set_neovide_refresh_rate()
	local max_rate, err = get_max_refresh_rate()
	if max_rate then
		vim.g.neovide_refresh_rate = max_rate
		vim.notify("Neovide refresh rate set to: " .. max_rate .. "Hz")
	else
		vim.notify("Warning: " .. (err or "Could not detect refresh rate. Using default."))
	end
end

set_neovide_refresh_rate()
